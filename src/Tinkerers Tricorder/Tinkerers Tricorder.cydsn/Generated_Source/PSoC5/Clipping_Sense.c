...
uint8 Clipping_Sense_initVar = 0u;

/* Internal functions definitoin */
static void Clipping_Sense_trimAdjust(uint8 nibble) ;

/* static Clipping_Sense_backupStruct  Clipping_Sense_backup; */
#if (CY_PSOC5A)
    static Clipping_Sense_LOWPOWER_BACKUP_STRUCT  Clipping_Sense_lowPowerBackup;
#endif /* CY_PSOC5A */

/* variable to decide whether or not to restore the control register in 
   Enable() API for PSoC5A only */
#if (CY_PSOC5A)
    static uint8 Clipping_Sense_restoreReg = 0u;
#endif /* CY_PSOC5A */


/*******************************************************************************
* Function Name: Clipping_Sense_Init
********************************************************************************
*
* Summary:
...
void Clipping_Sense_Init(void) 
{
    ...
    Clipping_Sense_SetSpeed(Clipping_Sense_DEFAULT_SPEED);

    /* Set default Hysteresis */
    #if ( Clipping_Sense_DEFAULT_HYSTERESIS == 0u )
        Clipping_Sense_CR |= Clipping_Sense_HYST_OFF;
    #else
        Clipping_Sense_CR &= (uint8)(~Clipping_Sense_HYST_OFF);
    #endif /* Clipping_Sense_DEFAULT_HYSTERESIS == 0u */
    /* Power down override feature is not supported for PSoC5A. */
    #if (CY_PSOC3 || CY_PSOC5LP)
        /* Set default Power Down Override */
        #if ( Clipping_Sense_DEFAULT_PWRDWN_OVRD == 0u )
            Clipping_Sense_CR &= (uint8)(~Clipping_Sense_PWRDWN_OVRD);
        #else 
            Clipping_Sense_CR |= Clipping_Sense_PWRDWN_OVRD;
        #endif /* Clipping_Sense_DEFAULT_PWRDWN_OVRD == 0u */
    #endif /* CY_PSOC3 || CY_PSOC5LP */
    
    /* Set mux always on logic */
    Clipping_Sense_CR |= Clipping_Sense_MX_AO;

    /* Set default sync */
    Clipping_Sense_CLK &= (uint8)(~Clipping_Sense_SYNCCLK_MASK);
    #if ( Clipping_Sense_DEFAULT_BYPASS_SYNC == 0u )
        Clipping_Sense_CLK |= Clipping_Sense_SYNC_CLK_EN;
    #else
        Clipping_Sense_CLK |= Clipping_Sense_BYPASS_SYNC;
    #endif /* Clipping_Sense_DEFAULT_BYPASS_SYNC == 0u */
}


/*******************************************************************************
* Function Name: Clipping_Sense_Enable
********************************************************************************
*
* Summary:
...
{
    Clipping_Sense_PWRMGR |= Clipping_Sense_ACT_PWR_EN;
    Clipping_Sense_STBY_PWRMGR |= Clipping_Sense_STBY_PWR_EN;
     
     /* This is to restore the value of register CR which is saved 
    in prior to the modification in stop() API */
    #if (CY_PSOC5A)
        if(Clipping_Sense_restoreReg == 1u)
        {
            Clipping_Sense_CR = Clipping_Sense_lowPowerBackup.compCRReg;

            /* Clear the flag */
            Clipping_Sense_restoreReg = 0u;
        }
    #endif /* CY_PSOC5A */
}
...
/*******************************************************************************
* Function Name: Clipping_Sense_Start
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  Clipping_Sense_initVar: Is modified when this function is called for the 
...
void Clipping_Sense_Start(void) 
{
    ...
    if ( Clipping_Sense_initVar == 0u )
    {
        Clipping_Sense_Init();
        
        Clipping_Sense_initVar = 1u;
    }   
...
/*******************************************************************************
* Function Name: Clipping_Sense_Stop
********************************************************************************
*
* Summary:
...
void Clipping_Sense_Stop(void) 
{
    ...
    Clipping_Sense_PWRMGR &= (uint8)(~Clipping_Sense_ACT_PWR_EN);
    Clipping_Sense_STBY_PWRMGR &= (uint8)(~Clipping_Sense_STBY_PWR_EN);
    
    #if (CY_PSOC5A)
        /* Enable the variable */
        Clipping_Sense_restoreReg = 1u;

        /* Save the control register before clearing it */
        Clipping_Sense_lowPowerBackup.compCRReg = Clipping_Sense_CR;
        Clipping_Sense_CR = Clipping_Sense_COMP_REG_CLR;
    #endif /* CY_PSOC5A */
}


/*******************************************************************************
* Function Name: Clipping_Sense_SetSpeed
********************************************************************************
*
* Summary:
...
void Clipping_Sense_SetSpeed(uint8 speed) 
{
    ...
    Clipping_Sense_CR = (Clipping_Sense_CR & (uint8)(~Clipping_Sense_PWR_MODE_MASK)) |
                           (speed & Clipping_Sense_PWR_MODE_MASK);

    /* Set trim value for high speed comparator */
    if(speed == Clipping_Sense_HIGHSPEED)
    {
        /* PSoC5A */
        #if (CY_PSOC5A)
            Clipping_Sense_TR = Clipping_Sense_HS_TRIM_TR0;
        #endif /* CY_PSOC5A */
        
        /* PSoC3, PSoC5LP or later */
        #if (CY_PSOC3 || CY_PSOC5LP) 
            Clipping_Sense_TR0 = Clipping_Sense_HS_TRIM_TR0;
            Clipping_Sense_TR1 = Clipping_Sense_HS_TRIM_TR1;
        #endif /* CY_PSOC3 || CY_PSOC5LP */
    }
    else
    {
    /* PSoC5A */
        #if (CY_PSOC5A)
            Clipping_Sense_TR = Clipping_Sense_LS_TRIM_TR0;
        #endif /* CY_PSOC5A */
        
        /* PSoC3, PSoC5LP or later */
        #if (CY_PSOC3 || CY_PSOC5LP) 
            Clipping_Sense_TR0 = Clipping_Sense_LS_TRIM_TR0;
            Clipping_Sense_TR1 = Clipping_Sense_LS_TRIM_TR1;
        #endif /* CY_PSOC3 || CY_PSOC5LP */
    }
...
/*******************************************************************************
* Function Name: Clipping_Sense_GetCompare
********************************************************************************
*
* Summary:
...
{
    return( Clipping_Sense_WRK & Clipping_Sense_CMP_OUT_MASK);
}


/*******************************************************************************
* Function Name: Clipping_Sense_trimAdjust
********************************************************************************
*
* Summary:
...
static void Clipping_Sense_trimAdjust(uint8 nibble) 
{
    ...
    cmpState = Clipping_Sense_WRK & Clipping_Sense_CMP_OUT_MASK;
    ...
    {    
        ...
        if ( cmpState != 0u )
        {
            /* PSoC5A */
            #if (CY_PSOC5A)
                Clipping_Sense_TR |= Clipping_Sense_CMP_TRIM1_DIR;
            #endif /* CY_PSOC5A */
            
            /* PSoC3, PSoC5LP or later */
            #if (CY_PSOC3 || CY_PSOC5LP)
                Clipping_Sense_TR0 |= Clipping_Sense_CMP_TR0_DIR;
            #endif /* CY_PSOC3 || CY_PSOC5LP */
        }
    }
    else
    {
        ...
        if ( cmpState == 0u )
        {
            /* PSoC5A */
            #if (CY_PSOC5A)
                Clipping_Sense_TR |= Clipping_Sense_CMP_TRIM2_DIR; 
            #endif /* CY_PSOC5A */
            
            /* PSoC3, PSoC5LP or later */
            #if (CY_PSOC3 || CY_PSOC5LP)
                Clipping_Sense_TR1 |= Clipping_Sense_CMP_TR1_DIR;
            #endif /* CY_PSOC3 || CY_PSOC5LP */
        }
    }
    ...
    for ( trimCnt = 0u; trimCnt < trimCntMax; trimCnt++ )
	{
        if (nibble == 0u)
        {
            /* PSoC5A */
            #if (CY_PSOC5A)
                Clipping_Sense_TR += 1u;
            #endif /* CY_PSOC5A */
            
            /* PSoC3, PSoC5LP or later */
            #if (CY_PSOC3 || CY_PSOC5LP)
                Clipping_Sense_TR0 += 1u;
            #endif /* CY_PSOC3 || CY_PSOC5LP */
        }
        else
        {
            /* PSoC5A */
            #if (CY_PSOC5A)
                Clipping_Sense_TR += 0x10u;
            #endif /* CY_PSOC5A */
            
            /* PSoC3, PSoC5LP or later */
            #if (CY_PSOC3 || CY_PSOC5LP)
                Clipping_Sense_TR1 += 1u;
            #endif /* CY_PSOC3 || CY_PSOC5LP */
        }
        ...
        if ((Clipping_Sense_WRK & Clipping_Sense_CMP_OUT_MASK) != cmpState)
        {
...
/*******************************************************************************
* Function Name: Clipping_Sense_ZeroCal
********************************************************************************
*
* Summary:
...
uint16 Clipping_Sense_ZeroCal(void) 
{
    ...
    tmpSW0 = Clipping_Sense_SW0;
    tmpSW2 = Clipping_Sense_SW2;
    tmpSW3 = Clipping_Sense_SW3;

     /* Clear routing for inP, retain routing for inN */
    Clipping_Sense_SW0 = 0x00u;
    Clipping_Sense_SW2 = 0x00u;
    Clipping_Sense_SW3 = tmpSW3 & (uint8)(~Clipping_Sense_CMP_SW3_INPCTL_MASK);

    /* Preserve original configuration
     * - turn off Hysteresis
     * - set calibration bit - shorts inN to inP
    */
    tmpCR = Clipping_Sense_CR;
    Clipping_Sense_CR |= (Clipping_Sense_CAL_ON | Clipping_Sense_HYST_OFF);
    
    /* Write default low values to trim register - no offset adjust */
    /* PSoC5A */
    #if (CY_PSOC5A)
        Clipping_Sense_TR = Clipping_Sense_DEFAULT_CMP_TRIM;
    #endif /* CY_PSOC5A */
    
    /* PSoC3, PSoC5LP or later */
    #if (CY_PSOC3 || CY_PSOC5LP)
        Clipping_Sense_TR0 = Clipping_Sense_DEFAULT_CMP_TRIM;
        Clipping_Sense_TR1 = Clipping_Sense_DEFAULT_CMP_TRIM;
    #endif /* CY_PSOC3 || CY_PSOC5LP */
	
	/* Two phase trim - slow modes, one phase trim - for fast */ 
    if ( (Clipping_Sense_CR & Clipping_Sense_PWR_MODE_MASK) == Clipping_Sense_PWR_MODE_FAST)
    {
        Clipping_Sense_trimAdjust(0u);
    }
    else /* default to trim for fast modes */
    {
        Clipping_Sense_trimAdjust(1u);
		Clipping_Sense_trimAdjust(0u);
    }
   
    /* Restore Config Register */
    Clipping_Sense_CR = tmpCR;
    
    /* Restore routing registers for inP */
    Clipping_Sense_SW0 = tmpSW0;
    Clipping_Sense_SW2 = tmpSW2;
    Clipping_Sense_SW3 = tmpSW3;
    
    /* PSoC5A */
    #if (CY_PSOC5A)
        return (uint16) Clipping_Sense_TR;
    #endif /* CY_PSOC5A */
    
    /* PSoC3, PSoC5LP or later */
    #if (CY_PSOC3 || CY_PSOC5LP)
        return ((uint16)((uint16)Clipping_Sense_TR1 << 8u) | (Clipping_Sense_TR0));        
    #endif /* CY_PSOC3 || CY_PSOC5LP */
}


/*******************************************************************************
* Function Name: Clipping_Sense_LoadTrim
********************************************************************************
*
* Summary:
...
void Clipping_Sense_LoadTrim(uint16 trimVal) 
{
    ...
    /*******************************************************************************
    * Function Name: Clipping_Sense_PwrDwnOverrideEnable
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    /*******************************************************************************
    * Function Name: Clipping_Sense_PwrDwnOverrideDisable
    ********************************************************************************
    *
    * Summary:
