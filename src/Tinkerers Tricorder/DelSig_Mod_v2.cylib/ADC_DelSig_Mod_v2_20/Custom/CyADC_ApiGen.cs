...
namespace ADC_DelSig_Mod_v2_20
{
    partial class CyCustomizer : ICyAPICustomize_v1
    {
        ...
        string []DSM_OUT1_ParamName = {"DFLT_DSM_OUT1", "DFLT_DSM_OUT1_CFG2", "DFLT_DSM_OUT1_CFG3", 
                                          "DFLT_DSM_OUT1_CFG4"};
        string []clockFrequency_ParamName = {"DFLT_CLK_FREQ", "DFLT_CLK_FREQ_CFG2", "DFLT_CLK_FREQ_CFG3", 
                                                "DFLT_CLK_FREQ_CFG4"};
                                                           ...
                                                           IEnumerable<CyAPICustomizer> apis)
        {
            ...
            float vdda = float.Parse(query.GetCommittedParam("Vdda_Value").Value);

            // Calculate values
            CalcValues(ref paramDict, isPSoC3_ES3, vdda);
...
/// <param name="isPSOC3_ES3">Flag to identify PSoC3 ES3 or later silicon</param>
void SetResolution(uint resolution, uint inputRange, uint config, bool isPSOC3_ES3)
{
    uint rShiftOffset = 0;
   ...
   // DecGvalReg[config]   = 0;
    ...
    if ((ADC_Input_Range[config] == Cyrc.ADC_IR_VSSA_TO_VREF) | 
        ...
        ((ADC_Input_Range[config]== Cyrc.ADC_IR_VSSA_TO_2VREF) && (isPSOC3_ES3 == false)))
    {
        rShiftOffset = 1;
    }
...
/// <param name="config">configuration selected.</param>
/// <param name="isPSOC3_ES3">silicon selected.</param>
/// <param name="vdda">vdda value entered in the system wide resource file. Based on this value
///                   reference will be selected.</param>
void SetRef(uint refMode, uint inputMode, uint config, bool isPSOC3_ES3, float vdda)
{
    // Mask off reference 
...
/// <param name="config">ADC configuration number.</param>
/// <param name="isPSOC3_ES3">Flag to identify the silicon version selected is PSoC3 or later</param>
...
void SetInputRange(uint inputMode, float refVoltage, uint config, bool isPSOC3_ES3, float vdda)
{
    //uint CapRatio;
    ...
    {
        if ((vdda < MAX_VDDA3_VALUE) && (inputMode == Cyrc.ADC_IR_VSSA_TO_VDDA) && isPSOC3_ES3)
        {
    ...
    switch (inputMode)
    {
        ...
        case Cyrc.ADC_IR_VSSA_TO_2VREF: // Single ended, Vssa to 2*Vref
            ...
            {            
                SetBuffer(Cyrc.BUF_BYPASS_NEG | Cyrc.BUF_RAIL_TO_RAIL, config);
                // Set reference mux to 1.024V reference
                DsmRef0Reg[config] &= ~Cyrc.DSM_REFMUX_MASK;
                DsmRef0Reg[config] |= Cyrc.DSM_REFMUX_1_024V;
                ...
                // Set resolution constants from table 
                // For 0 to 2*Vref mode with PSoC3 ES2/PSoC5 ES1 silicon
...
/// <param name="isPSOC3_ES3"></param>
/// <param name="vdda"></param>
void CalcValues(ref Dictionary<string, string> paramDict, bool isPSOC3_ES3, float vdda)
     {
            uint tmp;
            uint i = 1;
            string tmpString = "";
...
//            }
            ...
            for (i = 0; i < Cyrc.MAX_CONFIGS; i++)
            {
               ...
               // GVAL reg setting based on Resolution 
                ...
                if (ADC_Resolution[i] > 15)    // 16 to 20 bit Resolution
                {
                    ...
                    if ((vdda < MAX_VDDA3_VALUE) && (ADC_Input_Range[i] == Cyrc.ADC_IR_VSSA_TO_VDDA) && isPSOC3_ES3)
                    {
                ...
                SetResolution(ADC_Resolution[i], ADC_Input_Range[i], i, isPSOC3_ES3);
                SetBufferMode(Input_Buffer_Mode[i], i);
                if (Input_Buffer_Mode[i] != 0)
                {
                     SetBufferGain(Input_Buffer_Gain[i], i);
                }
                SetInputRange(ADC_Input_Range[i], Ref_Voltage[i], i, isPSOC3_ES3, vdda);
                SetRef(ADC_Reference[i], ADC_Input_Range[i], i, isPSOC3_ES3, vdda);
                ...
                if (ADC_Input_Range[i] == Cyrc.ADC_IR_VSSA_TO_2VREF && isPSOC3_ES3 == false)
                {
                ...
                paramDict.Add(IdealOddDecGain_ParamName[i], "0x" + Ideal_OddDec_Gain[i].ToString("X4") + "u");
    ...
    public static class CyresSettings
    {
            ...
            // Added these as a workaournd for 0 to 2*Vref mode with PSoC3 ES2/PSoC5 ES1 silicon 
            //   With PSoC3 ES2/PSoC5 ES1 silicon, the -ve input of DSM is connected to Vssa instead of
    ...
    public static class Cyrc
    {
       ...
       // Bit Field  DSM_FCHOP                    
        ...
        public const uint DSM_REFMUX_VDAC0_ES3  = 0x03;  // For PSoC3 ES3 and PSoC5 ES2 or later
        public const uint DSM_REFMUX_VDAC0  = 0x01;  // For PSoC3 ES2 and PSoC5 ES1
        
        public const uint DSM_REFMUX_UVCM   = 0x01;  // For PSoC3 ES2 or PSoC5 ES1         
        public const uint DSM_REFMUX_VDA_4  = 0x02;        
        public const uint DSM_REFMUX_VDA_3  = 0x03;  // For PSoC3 ES3 and PSoC5 ES2 or later        
