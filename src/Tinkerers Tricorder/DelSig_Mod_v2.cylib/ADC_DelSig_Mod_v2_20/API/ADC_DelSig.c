...
void `$INSTANCE_NAME`_Enable(void) `=ReentrantKeil($INSTANCE_NAME . "_Enable")`
{
    ...
    *
    ***************************************************************************/
    void `$INSTANCE_NAME`_RestorePowerRegisters(void)
    {
    ...
    *
    **************************************************************************/
    void `$INSTANCE_NAME`_SavePowerRegisters(void)
    {
    ...
    *
    ***************************************************************************/
    void `$INSTANCE_NAME`_SetLowPower(void)
    {
...
int16 `$INSTANCE_NAME`_CountsTo_mVolts( int32 adcCounts) `=ReentrantKeil($INSTANCE_NAME . "_CountsTo_mVolts")`
{
    ...
    if(`$INSTANCE_NAME`_`$Config1_Name`_RESOLUTION < 17)
    {
        A = 1000;
        B = 1;
    }
...
int32 `$INSTANCE_NAME`_CountsTo_uVolts( int32 adcCounts) `=ReentrantKeil($INSTANCE_NAME . "_CountsTo_uVolts")`
{
    ...
    if(`$INSTANCE_NAME`_`$Config1_Name`_RESOLUTION < 12)
    {
        A = 1000000;
        B = 1;
    }
    else if(`$INSTANCE_NAME`_`$Config1_Name`_RESOLUTION < 15)
    {
        A = 100000;
        B = 10;
    }
    else if(`$INSTANCE_NAME`_`$Config1_Name`_RESOLUTION < 18)
    {
        A = 10000;
        B = 100;
    }
...
void `$INSTANCE_NAME`_InitConfig(uint8 config)
{
    ...
    if (config == 1u)
    {
        ...
        `$INSTANCE_NAME`_DSM_REF0_REG    = `$INSTANCE_NAME`_`$Config1_Name`_DSM_REF0;    
        ...
        */
        ...
        if(config == 2u)
        {
            ...
            `$INSTANCE_NAME`_DSM_REF0_REG    = `$INSTANCE_NAME`_`$Config2_Name`_DSM_REF0;    
        ...
        if(config == 3u)
        {
            ...
            `$INSTANCE_NAME`_DSM_REF0_REG    = `$INSTANCE_NAME`_`$Config3_Name`_DSM_REF0;    
        ...
        if (config == 4u)
        {
            ...
            `$INSTANCE_NAME`_DSM_REF0_REG    = `$INSTANCE_NAME`_`$Config4_Name`_DSM_REF0;    
    ...
    {   
       /* Set the flag to ensure start() API dont override the config
        ...
        if(config == 1u)
        {
            ...
            clockDivider = `$INSTANCE_NAME`_RoundValue((uint32)BCLK__BUS_CLK__HZ,
